...
export async function purgeUserEvidence(env: Env, userId: number): Promise<void> {
  ...
  for (const row of results) {
    ...
    catch { console.error(`계정 삭제 중 R2 증빙 정리 실패: user=${userId} key=${row.evidence_object_key}`); }
  }
}

/** Permanently deletes a user and every row that references it (D1 FKs cascade). Call purgeUserEvidence first. */
export async function purgeUser(env: Env, userId: number): Promise<void> {
  await purgeUserEvidence(env, userId);
